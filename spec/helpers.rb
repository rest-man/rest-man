...
module Helpers
  ...
  # SimpleRestClient::Response object.
  #
  # @param net_http_res_double an rspec double for Net::HTTPResponse
  # @param request A SimpleRestClient::Request or rspec double
  #
  # @return [SimpleRestClient::Response]
  #
  def response_from_res_double(net_http_res_double, request=nil, duration: 1)
    request ||= request_double()
    start_time = Time.now - duration

    response = SimpleRestClient::Response.create(net_http_res_double.body, net_http_res_double, request, start_time)
  ...
  # Create a double for SimpleRestClient::Request
  def request_double(url: 'http://example.com', method: 'get')
    instance_double('SimpleRestClient::Request',
