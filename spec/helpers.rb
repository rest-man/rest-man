...
module Helpers
  ...
  # RestMan::Response object.
  #
  # @param net_http_res_double an rspec double for Net::HTTPResponse
  # @param request A RestMan::Request or rspec double
  #
  # @return [RestMan::Response]
  #
  def response_from_res_double(net_http_res_double, request=nil, duration: 1)
    request ||= request_double()
    start_time = Time.now - duration

    response = RestMan::Response.create(net_http_res_double.body, net_http_res_double, request, start_time)
  ...
  # Create a double for RestMan::Request
  def request_double(url: 'http://example.com', method: 'get')
    instance_double('RestMan::Request',
