...
describe SimpleRestClient::AbstractResponse, :include_helpers do

  # Sample class implementing AbstractResponse used for testing.
  class MyAbstractResponse

    include SimpleRestClient::AbstractResponse
  ...
  describe '.beautify_headers' do
    it "beautifies the headers by turning the keys to symbols" do
      h = SimpleRestClient::AbstractResponse.beautify_headers('content-type' => [ 'x' ])
      expect(h.keys.first).to eq :content_type
    end

    it "beautifies the headers by turning the values to strings instead of one-element arrays" do
      h = SimpleRestClient::AbstractResponse.beautify_headers('x' => [ 'text/html' ] )
      expect(h.values.first).to eq 'text/html'
    end

    it 'joins multiple header values by comma' do
      expect(SimpleRestClient::AbstractResponse.beautify_headers(
        {'My-Header' => ['one', 'two']}
  ...
  describe "#return!" do
    ...
    it "should raise RequestFailed on unknown codes" do
      ...
      expect { @response.return! }.to raise_error SimpleRestClient::RequestFailed
    end
    ...
    it "should gracefully handle 302 redirect with no location header" do
      ...
      @response = MyAbstractResponse.new(@net_http_res, @request)
      expect(@response).to receive(:check_max_redirects).and_return('fake-check')
      expect { @response.return! }.to raise_error SimpleRestClient::Found
    end
  end
end
